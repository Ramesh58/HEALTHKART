$(document).ready(function(){
	...
	if(location=="http://localhost:8081/health/" || location=="http://localhost:8080/health/home"){
		$('a[href*="/home"] .on').css('color','white');
	}
	if(location=="http://localhost:8081/health/login?logout"){
		$('a[href*="/login"] .on').css('color','white');
	}
	if(location=="http://localhost:8081/health/all/product/getallproducts"){
		$('a[href*="/getallproducts"] .on').css('color','white');
	}
	if(location=="http://localhost:8081/health/admin/product/productform"){
